...
# Rework SAP 2 Mentity to pull from SAP.Hierarchy
...
# Identify Duplicate MEntity values and Append existing list for Unique Values ----
  test_duplicate <-
    test[[1]][[5]]
  
nchar(test_duplicate)
...
```{r}
  ...
  mentity_extract <- function(x){
    y = x[["MEntity"]]
    return(y)
  }

    # Return list of profit centers with more than one MEntity number ----
    extract_multiple_MEntity <- function(x){
      y = list.filter(x, nchar(x[[1]]$MEntity) > 11)
      return(y)
    }
