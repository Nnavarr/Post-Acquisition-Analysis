...
```{r}
  ...
  # Nested profit center list ----
    nested_profit_center_list <- 
      list(as.list(quarter_list[[1]]),
           as.list(quarter_list[[2]]),
           as.list(quarter_list[[3]]),
           as.list(quarter_list[[4]]),
           as.list(quarter_list[[5]]),
           as.list(quarter_list[[6]]),
           as.list(quarter_list[[7]]),
           as.list(quarter_list[[8]]),
           as.list(quarter_list[[9]]),
           as.list(quarter_list[[10]]),
           as.list(quarter_list[[11]]),
           as.list(quarter_list[[12]]),
           as.list(quarter_list[[13]]),
           as.list(quarter_list[[14]]))

  # income statement data check ----
    income_statement_check <- at_depth(nested_profit_center_list, 2, DataCheck)
...
# ----------------------------------------------------------------------------------------
  # Nested list trial 2
   ...
   list_length <- length(q_test)
   
   # Container 
     list_test_container <- vector("list", length = list_length)
...
# Income statement compilation ----
  ...
  # Creation of group identifier table ----
    ...
    # Enter grp name identifier ----
      for (i in 1:length(aggregate_income_statement)) { 
