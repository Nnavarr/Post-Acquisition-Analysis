...
# Import Excel File ----
acq_list_final <- readxl::read_excel("\\\\adfs01.uhi.amerco\\departments\\mia\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Report\\Quarterly Acquisitions\\F19 Q4\\New Acquisitions List\\Master_Acquisitions_List.xlsx")
...
# Occ Data 
  occ_df =
    wss_data %>%
      filter(`MEntity` %in% x[["MEntity"]] & unm_product != 'UBOX' & Date <= '2019-03-01') %>% 
      group_by(MEntity,
               Date, 
               unm_product) %>%
...
```{r Income Statement Export, eval=FALSE, include=FALSE}
