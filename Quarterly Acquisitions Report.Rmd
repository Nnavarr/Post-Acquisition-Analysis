...
# Import Excel File ----
acq_list_final <- readxl::read_excel("\\\\adfs01.uhi.amerco\\departments\\mia\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\Acq List\\f20_master_list.xlsx")
...
line_item_function <- function(x, df){
    ...
    # current_month = Max IS Month in full date format 
     ...
     max_is_month <- as.Date('2019-06-01')
...
# Occ Data 
  occ_df =
    wss_data %>%
      filter(`MEntity` %in% x[["MEntity"]] & unm_product != 'UBOX' & Date <= '2019-06-01') %>% 
      group_by(MEntity,
               Date, 
               unm_product) %>%
...
# Collapse Unit Mix Process ----
 unit_mix_list <- modify_depth(q_acquisitions_list, 2, nested_list_filter, z = 5) 
...
# write.csv(unit_mix_final, 'Z:\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\Dashboard Connections\\unit_mix_final.csv') 
# 
...
write.csv(fc_occ_df_2, 'Z:\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\Dashboard Connections\\forecasted_occ.csv')
