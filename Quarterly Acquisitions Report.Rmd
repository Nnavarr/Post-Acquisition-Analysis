...
# Import Excel File ----
acq_list_final <- readxl::read_excel("\\\\adfs01.uhi.amerco\\departments\\mia\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\F20 Q3\\center_list\\acquisitions_list_final.xlsx")
...
IS.Trend.Cash <- dplyr::tbl(src_dbi(con), in_schema("FinAccounting.dbo", "IS_TREND_CASH"))
sap_uhi_group <- dplyr::tbl(src_dbi(con), in_schema("FinAccounting.dbo", "SAP_IS_UHAL_GROUP"))
wss_data <- dplyr::tbl(src_dbi(con), in_schema("Storage.dbo", "WSS_UnitMixUHI_Monthly_Archive"))
forecasted_occ_db <- dplyr::tbl(src_dbi(con), in_schema("FinAnalysis.dbo", "Storage_Forecast"))
  forecasted_occ_db <- collect(forecasted_occ_db)
  forecasted_occ_db$FC_Date <- as.Date(forecasted_occ_db$FC_Date, format = "%Y-%m-%d")
  forecasted_occ_db$Start_Date <- as.Date(forecasted_occ_db$Start_Date, format = "%Y-%m-%d")
  forecasted_occ_db$Upload_Date <- as.Date(forecasted_occ_db$Upload_Date, format = "%Y-%m-%d")


SAP.Hierarchy <- dplyr::tbl(src_dbi(con), in_schema("FinAccounting.dbo", "SAP_Cost_Center_Hierarchy"))
...
# SAP Accounts ----
accounts <-
  read.csv("\\\\adfs01.uhi.amerco\\departments\\mia\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\income_statement_accounts.csv")
...
```{r Line Item Function, eval=FALSE, include=FALSE}
...
line_item_function <- function(x, df){
    ...
    # current_month = Max IS Month in full date format 
     ...
     max_is_month <- as.Date('2019-09-01')
...
```{r Aggregate Income Statement, eval=FALSE, include=FALSE}
...
# Import Fiscal Year Columns ----
  ...
  for (i in 1:length(db$FY)){
    ...
    } else if (db$MONTH[i] >= '2019/4/1' & db$MONTH[i] <= '2019/6/30'){
                  
      db$FY[i] = 2020
      db$Quarter[i] = 1                
              
    } else if (db$MONTH[i] >= '2019/7/1' & db$MONTH[i] <= '2019/9/30'){
                  
      db$FY[i] = 2020
      db$Quarter[i] = 2
              
    } else if (db$MONTH[i] >= '2019/10/1' & db$MONTH[i] <= '2019/12/30'){
                  
      db$FY[i] = 2020
      db$Quarter[i] = 3
              
    } else if (db$MONTH[i] >= '2020/1/1' & db$MONTH[i] <= '2020/3/31'){
...
```{r Individual Income Statement, eval=FALSE, include=FALSE}
...
```{r Minimum IS Trend Data, eval=FALSE, include=FALSE}
...
```{r Income Statement Data Check, eval=FALSE, include=FALSE}
...
```{r Nested List Creation, include=FALSE}
...
# Occ Data 
  occ_df =
    wss_data %>%
      filter(`MEntity` %in% x[["MEntity"]] & unm_product != 'UBOX' & Date <= '2019-12-01') %>% 
      group_by(MEntity,
               Date, 
               unm_product) %>%
...
db <-
  forecasted_occ_db %>%
  dplyr::filter(MEntity %in% x[["MEntity"]]) %>%
  dplyr::group_by(MEntity)
  ...
  # Rename Data Column to "MONTH"
      colnames(db)[5] <- "MONTH"
        ...
        # Create Fiscal Year Columns for Data 
        if (nrow(db_filtered) > 1){
          db_filtered <- 
            fiscal_year_columns(db_filtered)
...
# Append List 
    y =
      append(x, list(db_filtered))
  
  names(y)[[5]] = "Forecasted_Occupancy"
...
acq_list_final$Group <- as.numeric(acq_list_final$Group)
...
```{r Aggregate Income Statement Process Complete 1 4 2019, eval=FALSE, include=FALSE}
...
is_collapsed <- 
  left_join(is_collapsed, grp_table, by = "Group")  
...
# Collapse Unit Mix Process ----
 unit_mix_list <- modify_depth(q_acquisitions_list, 2, nested_list_filter, z = 4) 
...
# MEntity Count by Group ----
unit_mix_final %>%
    group_by(Group) %>%
    select(MEntity) %>%
    summarise(n_distinct(MEntity))
...
duplicate_list <- q_acquisitions_list
...
forecast_only_list <- 
  modify_depth(duplicate_list, 2, nested_list_filter, z = "Forecasted_Occupancy") 
...
# Join ----
fc_occ_df_2 <-
  left_join(forecast_occ_final, occ_df_2[ , c(1,3)], by = c("MEntity"))
               
# Rename Forecasted Occupancy Column ----
colnames(fc_occ_df_2)[6] <- "Forecasted_Rooms"  

fc_occ_df_2 <- fc_occ_df_2[, c('ID', 'MEntity', 'MONTH', 'Forecasted_Rooms', 'Group', 'FY', 'FM', 'Quarter', 'Month', 'Total_Rooms')]
  colnames(fc_occ_df_2)[9] <- 'Month1'
  
  
# Forecasted Occ count ----
fc_occ_df_2 %>%
    group_by(Group) %>%
    select(MEntity) %>%
    summarize(n_distinct(MEntity))
...
```{r NA Forecasted OCC centers, eval=FALSE, include=FALSE}
...
# write.csv(is_collapsed, "\\\\adfs01.uhi.amerco\\departments\\mia\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\Dashboard Connections\\Income_Statement.csv", 
#           row.names = FALSE)
# 
# # Aggregate IS Export (For Traditional Income Statement / Waterfall view) ----
# write.csv(aggregate_IS_db, "\\\\adfs01.uhi.amerco\\departments\\mia\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\Dashboard Connections\\Aggregate_Income_Statement.csv", 
#           row.names = FALSE)

```


```{r Occupancy Metrics Export, eval=FALSE, include=FALSE}
# 
# xlsx::write.xlsx(unit_mix_final, 'Z:\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\Dashboard Connections\\UnitMix.xlsx',
#                   append = FALSE,
#                   sheetName = "UnitMix",
#                   col.names = TRUE,
#                   row.names = FALSE
...
write.csv(unit_mix_final, 'Z:\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\F20 Q3\\Data\\unitmix.csv')


# Aggregate Export ----
# write.csv(aggregate_occ, "\\\\adfs01.uhi.amerco\\departments\\mia\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\Dashboard Connections\\Aggregate_Occupancy.csv", 
#           row.names = FALSE)


```

```{r Forecasted Occ Metrics Export, eval=FALSE, include=FALSE}
...
# 1/28/2020 ----
write.csv(fc_occ_df_2, 'Z:\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\F20 Q3\\Data\\forecasted_occ.csv')




# Export to CSV to be pasted into XLSX ----

# Previously Used File path (before 10/29/2019)
# 'Z:\\group\\MIA\\Noe\\Projects\\Post Acquisition\\Quarterly Acquisitions\\Dashboard Connections\\forecasted_occ.csv'
