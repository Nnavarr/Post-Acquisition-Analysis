...
# Export Data
  write.csv(Data.Combined, 'Z:\\group\\MIA\\Noe\\Projects\\Same Store & Cap Ex\\Report\\Quarterly Acquisitions\\F19 Q1\\Report72018.csv', append = FALSE)
...
# Vector to filter Product Type #
  ...
  # From there, we can calculate our summary fields (Total_SF & Occupied_SF (a sum of these fields respectively)
    grp1.unitmix = Occupancy.Table %>% 
                   filter(`MEntity` %in% grp1.ME & unm_product %in% product.type & Date <= '2018-06-01') %>% 
                   group_by(MEntity, 
                        unm_timestamp, 
                        unm_product) %>%
...
To do: 7/26/2018
  ...
  Determine whether or not the list of Profit Centers match the IS Trend Data 
  
  
```{r}
  
group1.pc
Occ1.pc <- as.character(unlist(grp1.pc.list))

group2.pc
Occ2.pc <- as.character(unlist(grp2.pc.list))

group3.pc
Occ3.pc <- as.character(unlist(grp3.pc.list))

group4.pc
Occ4.pc <- as.character(unlist(grp4.pc.list))

group5.pc
Occ5.pc <- as.character(unlist(grp5.pc.list))

group6.pc
Occ6.pc <- as.character(unlist(grp6.pc.list))

group7.pc
Occ7.pc <- as.character(unlist(grp7.pc.list))

group8.pc
Occ8.pc <- as.character(unlist(grp8.pc.list))

group9.pc
Occ9.pc <- as.character(unlist(grp9.pc.list))

group10.pc
Occ10.pc <- as.character(unlist(grp10.pc.list))


group11.pc 
Occ11.pc <- as.character(unlist(grp11.pc.list))

group12.pc
Occ12.pc <- as.character(unlist(grp12.pc.list))

group13.pc
Occ13.pc <- as.character(unlist(grp13.pc.list))

group14.pc
Occ14.pc <- as.character(unlist(grp14.pc.list))
...
Compare to Previously Active UHI owned properties  
  
  
```{r Compare UHI New Acquisitions vs Old Centers}


# Graph File Info by Owner
  Graph.Complete.Info <- left_join(Graph.Entity.Info.Local, Graph.File[ , c(1, 2, 11)], by = c('MEntity', 'Entity'))


# Entire UHI (New Acq Included) #
  UHI.Profitcenter <- 
  Graph.Complete.Info  %>% 
    filter(Simple.Owner == 'UHI') %>%
    select(Profit_Center)

    n_distinct(UHI.Profitcenter$Profit_Center) # 1,365 Unique Profit Center numbers
    
    # Distinct Profit Centers
    UHI.Profitcenter.Distinct <- 
      UHI.Profitcenter %>%
        distinct(Profit_Center) %>%
        arrange(Profit_Center)
    
# ----------------------------------------------------------------------------------------------------- #
    
    # New Acq Profit Centers #
      New.Acq.Profitcenter <- unlist(Quarter.list)
      
        # Remove NA Observations
          New.Acq.Profitcenter <- New.Acq.Profitcenter[ !is.na(New.Acq.Profitcenter)] 
      
            n_distinct(New.Acq.Profitcenter) # 245 Unique profit centers
            
              # The list of profit centers above are already unique values #
        
                # DF Creation
    New.Acq.PC <- as.data.frame(New.Acq.Profitcenter)
    
      colnames(New.Acq.PC)[1] <- "Profit_Center"

            
            
            
              # ------------------------------------------------------------ #
              # How many of these new acq are present within the Graph file? #
              # ------------------------------------------------------------ #
            
                New.Acq.PC %>%
                  filter(Profit_Center %in% Graph.Entity.Info.Local$Profit_Center)
            
                # Only 155 are located within the Graph file out of 245.  The remaining properties are all new acquisitions that have not yet been assigned profit centers or simply too new. They will be added to the System total to obtain the full Pool.
                
                
# Addition of New Acquisitions Outside the Graph File
  UHI.Profitcenter.Distinct2 <- bind_rows(UHI.Profitcenter.Distinct, New.Acq.PC)
    ...
    UHI.ProfitCenters <- distinct(UHI.Profitcenter.Distinct2)

# ----------------------------------------------------------------------------------------------------- #            
            
# Exclude New Acq
  
  Excl.New.Acq.Profitcenters <-                     
  UHI.ProfitCenters %>% filter(!Profit_Center %in% New.Acq.Profitcenter)

       Excl.New.Acq.Profitcenters %>% summarise(n_distinct(Profit_Center)) #1,210 Unique Centers (includes 3 < 7000000000)
  
        # Convert to Vector
          Excl.New.Acq.Profitcenters <- as.vector(Excl.New.Acq.Profitcenters$Profit_Center)
...
```{r Income Statement Aggregation for Existing UHI}

  Excl.New.Acq.IS <- Aggregate.IS(Excl.New.Acq.Profitcenters) 

  # Export to csv
    write.csv(Excl.New.Acq.IS, "Z:\\group\\MIA\\Noe\\Projects\\Same Store & Cap Ex\\Report\\Quarterly Acquisitions\\F19 Q1\\Excluding_NewAcq_IS.csv")
...
```{r}
  ...
  # Vector Conversion
  UHI.Profitcenter.Vector <- as.vector(UHI.ProfitCenters$Profit_Center)

 
  UHI.IS <- Aggregate.IS(UHI.Profitcenter.Vector)
  
  # Export to CSV
    write.csv(UHI.IS, "Z:\\group\\MIA\\Noe\\Projects\\Same Store & Cap Ex\\Report\\Quarterly Acquisitions\\F19 Q1\\Total_UHI.csv")
...
New.Acq.Is <- Aggregate.IS(New.Acq.Profitcenter)

  # Write CSV
    write.csv(New.Acq.Is, "Z:\\group\\MIA\\Noe\\Projects\\Same Store & Cap Ex\\Report\\Quarterly Acquisitions\\F19 Q1\\New_Acq_UHI.csv")
...
# Alternative (UHI Pool)

This section pulls unique profit centers from all UHAUL control within the Trend IS Statement. 

```{r}

  Unique.UHI.Total <- read.csv("\\\\adfs01.uhi.amerco\\departments\\mia\\group\\MIA\\Noe\\Projects\\Same Store & Cap Ex\\Report\\Quarterly Acquisitions\\F19 Q1\\Unique UHI Profit Center.csv")


  Unique.UHI.Total$Profit_Center <- as.character(Unique.UHI.Total$Profit_Center)


  Unique.UHI.Total.Vector <- as.vector(Unique.UHI.Total$Profit_Center)


  
  Aggregate.Income.Statement <- Aggregate.IS(Unique.UHI.Total.Vector)

  
    write.csv(Aggregate.Income.Statement,"Z:\\group\\MIA\\Noe\\Projects\\Same Store & Cap Ex\\Report\\Quarterly Acquisitions\\F19 Q1\\Total_UHI.csv" )
...
SAP.Hierarchy %>% filter(`Profit Center` == '7000000013')

```
  
_________________________________________________________________________________________________________________________________________________

Rob's All AREC List of Centers

The goal in this code chunk is to determine whether the list used above is the same as Rob's list.  Is it a matter of data source that leads to discrepancies?  That is, does the 19 graph not contain a full picture of MEntity / Profit Centers found in other more complete data bases? 

```{r}

# All AREC Trend
  Robs_Centers <- read.csv("\\\\adfs01.uhi.amerco\\departments\\mia\\group\\MIA\\Noe\\Projects\\Same Store & Cap Ex\\Report\\Quarterly Acquisitions\\F19 Q1\\Rob's Comparison\\Robs Center List.csv")

  # Convert MEntity and Profit_Centers to Characters
    Robs_Centers$MEntity <- as.character(Robs_Centers$MEntity)
    Robs_Centers$Profit_Center <- as.character(Robs_Centers$Profit_Center)
    Robs_Centers$Owner <- as.character(Robs_Centers$Owner)
    Robs_Centers$Entity <- as.character(Robs_Centers$Entity)
    
      # Distinct Profit Centers
        Robs_Centers %>%
          summarise(MEntity_Count = n_distinct(Robs_Centers$MEntity),
                    Profit_Center_Count = n_distinct(Robs_Centers$Profit_Center))
        
          # Original Structure of DB
            # 1,508 <- Rows 
    
            # Match MEntity to Profit Centers from SAP DB
              Robs_Centers_MEntity <- unique(Robs_Centers$MEntity) # 1,420 Unique MEntity Numbers
                Robs_Centers_MEntity <- as.data.frame(Robs_Centers_MEntity)
                  colnames(Robs_Centers_MEntity)[1] <- "MEntity"
                
                  Robs_Centers_Mentity.Combined <- left_join(Robs_Centers_MEntity, SAP.Hierarchy[ ,c( 12,44, 2)], by = "MEntity")
                    
                    # Distinct Profit Center Count
                      Robs_Centers_Mentity.Combined %>%
                        summarise(n_distinct(`Profit Center`)) #1,420 Distinct Profit Centers, 1419 excluding NA category
                      
                      
                        # Rob's Distinct Profit Centers
                          Robs_Distinct_PC <- Robs_Centers_Mentity.Combined %>% 
                                                distinct(`Profit Center`) %>%
                                                filter(!is.na(`Profit Center`))
                        
# Comparison of Rob's List to New Acquisition Profit Center
#` In this section, we would like to see how many New Acquistion Profit Centers are contained within Rob's List `

  Robs_Distinct_PC %>% 
    filter(`Profit Center` %in% New.Acq.PC$Profit_Center) 
    
      # Fromt he line above, it seems only 218 are present. As a result, the better approach to compile the New Acquisiton vs System wide totals would be to Use the Graph File as the system total. Then, add the new aquisitions list (there might be some where the acquisition in too new to have show a profit center within the Graph file).  Once complete, we can take the distinct profit centers to create the system wide totals. 
  
  
  # ` This approach was originally taken early in the code chunks above. We now have verification to back-up this type of approach. 
