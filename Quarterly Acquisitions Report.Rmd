...
```{r Duplicate Profit Center Check}
...
```{r Check for Income Statement Representation}
...
# Append Income Statement To q_acquisitions_list ----
  q_acquisitions_list_append <-
   modify_depth(quarterly_acquisitions_list, 2, income_statment_compilation_append, accounts = sap_accounts$SAP_number)
...
```{r Remove Profit Centers without MEntity Numbers}
...
```{r NA Occupancy MEtrics - Infor on NA Occupancy MEntity}
...
```{r Collapse List Into Single Data Frames}
...
```{r Forecasted Occupancy Metrics}
...
```{r Forecasted Occ Inclusion}

in_fc <- 
  fc_occ_df_2 %>%
    filter(MEntity %in% fc_occ_df_2$MEntity)

# Group by Quarterly Acquisitions group ----
in_fc_summary <- 
  in_fc %>%
...
```{r Extract Observations Missing Forecasts}
...
```{r Extract NA Values, eval=FALSE, include=FALSE}
...
# Export List <----
#write.csv(acq_list_final, "Z:\\group\\MIA\\Noe\\Projects\\Same Store & Cap Ex\\Report\\Quarterly Acquisitions\\F19 Q2\\Acq_list_final.csv")

```

```{r Income Statement Export}

xlsx::write.xlsx(income_statement_aggregate, 
                 "Z:\\group\\MIA\\Noe\\Projects\\Same Store & Cap Ex\\Report\\Quarterly Acquisitions\\Dashboard Connections\\Income_Statement.xlsx",
                 sheetName = "Income_Statement")
```

```{r Occupancy Metrics Export}
...
```{r Forecasted Occupancy Inclusion Export}
...
```{r Forecasted Occ Metrics Export}
...
```{r Explore Missing Forecasts Export}
