...
# Import Fiscal Year Columns ----
  ...
  # Filter Date column (the database will often have slots ready for future dates. We do not want these.) ----
...
# If else statements ----
 ...
 # Calculate NOI ----
  if(db$`Line Item` %in% grepl("U-BOX", db$`Line Item`)){
  ...
  } else if(db$`Line Item` %in% grepl("STORAGE", db$`Line Item`)) {
    
  db <-   
   db %>%
   mutate(NOI = `STORAGE INCOME` + SALES + `MISCELLANEOUS INCOME` + UBox + `U-MOVE NET COMMISSION` - `COST OF SALES` - ADVERTISING - `LIABILITY INSURANCE` - PERSONNEL - `RENT-EQUIPMENT/LAND AND BLDGS` - `REPAIRS AND MAINTENANCE/GENERAL` - SUPPLIES - TELEPHONE - UTILITIES - `OTHER OPERATING EXPENSE`) 
    
  } else if(db$`Line Item` != 'SALES') {
    
   db <-   
   db %>%
   mutate(NOI = `MISCELLANEOUS INCOME` + UBox + `U-MOVE NET COMMISSION` - `COST OF SALES` - ADVERTISING - `LIABILITY INSURANCE` - PERSONNEL - `RENT-EQUIPMENT/LAND AND BLDGS` - `REPAIRS AND MAINTENANCE/GENERAL` - SUPPLIES - TELEPHONE - UTILITIES - `OTHER OPERATING EXPENSE`)   
