...
sap_uhi_group %>%
  filter(`PROFIT CENTER` %in% x & ACCOUNT %in% sap_accounts$ACCOUNT) 

  db = collect(sap_uhi_group)
...
test_function <- map(quarter_list, filter_test)
...
test_function <- map(q1, income_statment_compilation, accounts = sap_accounts$SAP_number)

test_df <- test_function[[1]]


  # Determine Negative value line items ----
  negative_df <-
    test_df[test_df$Value < 0, ]

    # Pull distinct Line Items ----
    negative_df %>%
      distinct(`Line Item`)
