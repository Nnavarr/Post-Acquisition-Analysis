...
def dlr01_prep():
    ...
    dlr01_query = "SELECT * FROM [MEntity].[dbo].[ENTITY_DLR01] WHERE [MEntity] in {}".format(tuple(new_list.MEntity))
    dlr01_df = pd.read_sql_query(dlr01_query, mentity_engine)
    dlr01_df = dlr01_df.sort_values(['MEntity', 'DATE_CLOSED']).drop_duplicates('MEntity', keep='last').copy()
    dlr01_df.DATE_EXPIRATION = dlr01_df.DATE_EXPIRATION.astype(str)

    # error check ----
    assert dlr01_df.DATE_EXPIRATION.all() == '9999-12-31', print("An center is not importing it's most recent Entity iteration")
    assert dlr01_df.STATUS.all() == 'O', print("Not all centers are showing a status of 'O' for Open")
    ...
    merge_df = pd.merge(left=new_list, right=dlr01_df[dlr01_cols], on='MEntity', how='left')
    drop_cols = ['Name', 'FY Closed', 'Quarter Closed', 'Acres', 'Improved SF', 'Entity_Current', 'Extract_Date']
...
def graph_prep(df):
    ...
    # initial connection / import ----
    mentity_engine = create_connection(database='MEntity')
    index_match_query = "SELECT * FROM [FINANALYSIS].[dbo].[GRAPH_INDEX_MATCH] " \
                        "WHERE [MEntity] in {}".format(tuple(df.MEntity))
    index_match_df = pd.read_sql_query(index_match_query, mentity_engine)
    index_match_df['Remote'] = index_match_df['Parent MEntity'].apply(lambda x: False if x is None else True)
    entity_info_query = "SELECT * FROM [FINANALYSIS].[dbo].[GRAPH_ENTITY_INFO] " \
    ...
    graph_df = pd.merge(index_match_df, entity_info_df[entity_cols], on='MEntity', how='left')
    merge_df = pd.merge(df, graph_df, on='MEntity', how='left', suffixes=['_newlist', '_graph'])
    ...
    merge_df['Include?'] = np.where(merge_df.Remote + merge_df.Abutting == 0, True, False)
...
def real_add_prep(df):
    ...
    merge_df = pd.merge(left=df, right=real_add_df[real_add_cols], on='MEntity', how='left')

    # select appropriate columns ----
    existing_cols = ['Group', 'Close of Escrow', 'ENTITY_NAME', 'District_newlist', 'MCO_newlist', 'Entity_newlist',
                     'Cost Center', 'Address', 'City', 'State_newlist', 'Latitude', 'Longitude', 'CBSA',
                     'Purchase Price',
                     'Property Description', 'Simple Owner', 'Include?', 'Construction_Type', 'MEntity',
                     'Parent MEntity',
                     'Abutting', 'Remote']
...
def final_col_rename(df):
    ...
    df.rename(columns={'District_newlist': 'DISTRICT_NO',
                               'MCO_newlist': 'MCO_NUM',
